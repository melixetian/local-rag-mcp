...
MCP_DECISION_PROMPT_PATH = Path(__file__).parent / "prompts" / "mcp_decision_prompt.txt"
...
class CompanyKBAssistant:
    ...
    def _llm_decide_mcp_usage(self, query: str, contexts):
        ...
        decision_prompt = Template(MCP_DECISION_PROMPT_PATH.read_text(encoding="utf-8")).substitute(
            query=query,
        ...
        try:
            response = self.llm_client.chat(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "You are a helpful assistant that decides when to use tools."
    ...
    def query(self, user_query: str, verbose=False):
        ...
        return {"answer": answer, "sources": sources, "mcp_used": mcp_result is not None, "mcp_tool": mcp_tool_used}
